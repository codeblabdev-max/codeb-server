...
// 데이터베이스 (간단한 JSON 파일)
const DB_FILE = process.env.DB_FILE || path.join(CONFIG.STORAGE_BASE, 'projects.json');
...
// Database push endpoint (execute SQL directly)
app.post('/api/projects/:name/db/push', async (req, res) => {
    const { name } = req.params;
    
    try {
        log('INFO', `Executing SQL push for project: ${name}`);
        
        let sqlFile = `/tmp/${name}_push.sql`;
        let sqlContent = '';
        
        // Handle different SQL sources
        if (req.files && req.files.sqlFile) {
            // Handle file upload
            const uploadedFile = req.files.sqlFile;
            await uploadedFile.mv(sqlFile);
            sqlContent = await fs.readFile(sqlFile, 'utf8');
        } else if (req.body.sql) {
            // Handle raw SQL in request body
            sqlContent = req.body.sql;
            await fs.writeFile(sqlFile, sqlContent);
        } else {
            return res.status(400).json({ 
                success: false, 
                error: 'No SQL provided. Use sqlFile upload or sql in request body.' 
            });
        }
        ...
        const dangerousOps = ['drop database', 'drop schema', 'truncate', 'delete from users'];
        const hasDangerous = dangerousOps.some(op => sqlLower.includes(op));
        
        if (hasDangerous && !req.body.force) {
            return res.status(400).json({
                success: false,
                error: 'SQL contains potentially destructive operations. Add "force": true to proceed.',
                warning: 'Detected: DROP DATABASE, DROP SCHEMA, TRUNCATE, or DELETE FROM users'
            });
        }
        
        // Copy SQL file to PostgreSQL container
        await execAsync(`
            podman cp ${sqlFile} ${name}-postgres:/tmp/push.sql
        `);
        ...
        const { stdout, stderr } = await execAsync(`
            podman exec ${name}-postgres sh -c "
                psql -U user -d ${name} -f /tmp/push.sql
            "
        `);
        ...
        await execAsync(`
            podman exec ${name}-postgres sh -c "rm -f /tmp/push.sql"
        `).catch(() => {});
        ...
        log('ERROR', 'SQL push failed', error);
        
        // Clean up on error
        try {
            await fs.unlink(`/tmp/${name}_push.sql`).catch(() => {});
        } catch {}
