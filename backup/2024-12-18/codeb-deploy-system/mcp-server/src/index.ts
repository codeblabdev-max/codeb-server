...
const tools = [
  ...
  // ====== SSOT (Single Source of Truth) 도구 ======
  {
    name: 'ssot_initialize',
    description: 'SSOT를 초기화합니다. 빈 SSOT를 생성하거나 기존 시스템에서 마이그레이션합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        force: {
          type: 'boolean',
          description: '기존 SSOT가 있어도 강제로 초기화',
        },
        migrateExisting: {
          type: 'boolean',
          description: '기존 레지스트리에서 마이그레이션 (기본값: true)',
        },
      },
    },
  },
  {
    name: 'ssot_migrate',
    description: '기존 시스템(port-manifest, project-registry)에서 SSOT로 마이그레이션합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        dryRun: {
          type: 'boolean',
          description: '실제 마이그레이션 없이 예상 결과만 확인',
        },
      },
    },
  },
  {
    name: 'ssot_validate',
    description: 'SSOT의 무결성을 검증합니다. 포트 충돌, 도메인 불일치, Caddy 설정 등을 확인합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        autoFix: {
  ...
  {
    name: 'ssot_get',
    description: '현재 SSOT 전체 상태를 조회합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        includeIndexes: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        gitRepo: { type: 'string', description: 'Git 저장소 URL' },
  ...
  },
  {
    ...
    description: 'SSOT에서 특정 프로젝트 정보를 조회합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string', description: '프로젝트 ID' },
  ...
  },
  {
    ...
    description: 'SSOT에 등록된 모든 프로젝트 목록을 조회합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        status: {
  ...
  },
  {
    ...
    description: '프로젝트에 도메인을 설정합니다. Caddy와 DNS가 자동으로 설정됩니다.',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string', description: '프로젝트 ID' },
        ...
        domain: { type: 'string', description: '도메인 (예: myapp.codeb.dev)' },
        targetPort: { type: 'number', description: '프록시 대상 포트' },
        prNumber: { type: 'string', description: 'Preview 환경 PR 번호' },
      },
  ...
  },
  {
    ...
    description: '프로젝트에서 도메인을 제거합니다. Caddy와 DNS 설정도 함께 제거됩니다.',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string', description: '프로젝트 ID' },
  ...
  },
  {
    ...
    description: '프로젝트에 포트를 할당합니다. 환경과 서비스 유형에 맞는 범위에서 자동 선택됩니다.',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string', description: '프로젝트 ID' },
  ...
  },
  {
    ...
    description: '특정 환경과 서비스에 대해 사용 가능한 포트를 찾습니다.',
    inputSchema: {
      type: 'object',
      properties: {
        environment: {
  ...
  },
  {
    ...
    description: 'SSOT 변경 이력을 조회합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        limit: {
          type: 'number',
          description: '조회할 이력 수 (기본값: 10)',
        },
      },
    },
  },
  {
    name: 'ssot_sync',
    description: 'SSOT를 기준으로 실제 시스템(Caddy, DNS, 컨테이너)을 동기화합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        dryRun: {
          type: 'boolean',
          description: '실제 동기화 없이 예상 결과만 확인',
        },
        components: {
          type: 'array',
          items: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      ...
      // ====== SSOT (Single Source of Truth) 핸들러 ======
...
async function main() {
  ...
  // 🔒 Protection Guard 초기화
  console.error('🔒 Protection Guard enabled');
