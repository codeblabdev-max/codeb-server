...
export class SSHClient {
  ...
  /**
   * 보호 기능 활성화/비활성화 (주의: 비활성화 시 위험!)
   */
  setProtection(enabled: boolean): void {
    if (!enabled) {
      console.error('⚠️ [SSHClient] WARNING: Protection disabled - USE WITH CAUTION!');
    }
  ...
  async exec(command: string, timeoutOrOptions: number | { timeout: number; skipProtection?: boolean } = 60000): Promise<SSHCommandResult> {
    ...
    if (this.protectionEnabled && !skipProtection) {
      ...
      if (!protectionResult.allowed) {
        ...
        return {
          stdout: '',
          stderr: `🛑 BLOCKED: ${protectionResult.warnings.join('\n')}`,
          code: 403,
          duration: 0,
          blocked: true,
      ...
      if (protectionResult.modifiedCommand) {
        console.error(`⚡ [SSHClient] Command modified for safety: ${command} → ${protectionResult.modifiedCommand}`);
