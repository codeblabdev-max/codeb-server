...
export async function analyzeServer(): Promise<ServerStatus> {
  ...
  try {
    // 병렬로 모든 정보 수집
    const [system, containers, pm2, ports, databases, registry] = await Promise.all([
      getSystemInfo(ssh),
      getContainers(ssh),
      getPM2Processes(ssh),
      getPortStatus(ssh),
      getDatabases(ssh),
      getRegistryStatus(ssh),
    ]);
...
async function getRegistryStatus(ssh: ReturnType<typeof getSSHClient>): Promise<RegistryStatus> {
  ...
  const containerCheck = await ssh.exec(
    'podman ps --filter name=registry --format "{{.Status}}" 2>/dev/null || echo ""'
  );

  const isAvailable = containerCheck.stdout.toLowerCase().includes('up');
  ...
  const imagesResult = await ssh.exec(
    'curl -s http://localhost:5000/v2/_catalog 2>/dev/null || echo "{\\"repositories\\":[]}"'
  );

  let images: RegistryStatus['images'] = [];
  try {
    const catalog = JSON.parse(imagesResult.stdout);
    for (const repo of catalog.repositories || []) {
      const tagsResult = await ssh.exec(
        `curl -s http://localhost:5000/v2/${repo}/tags/list 2>/dev/null || echo "{\\"tags\\":[]}"`
      );
