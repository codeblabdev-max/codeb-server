...
cmd_db() {
    ...
    case $subcommand in
        ...
        "push")
            ...
            local response=$(curl -s --max-time 180 -X POST \
                -F "sqlFile=@$sql_file" \
                "${API_BASE}/projects/${project_name}/db/push")
                
            local success=$(echo "$response" | jq -r '.success' 2>/dev/null)
            
            if [ "$success" = "true" ]; then
                local message=$(echo "$response" | jq -r '.message')
                local affected_rows=$(echo "$response" | jq -r '.affectedRows // "N/A"')
                log_success "SQL 실행 완료: $message"
                if [ "$affected_rows" != "N/A" ]; then
                    log_info "영향받은 행: $affected_rows"
                fi
            else
                local error=$(echo "$response" | jq -r '.error' 2>/dev/null)
                log_error "SQL 실행 실패: $error"
                return 1
            fi
            ;;
