---
allowed-tools: [Read, Write, Edit, Bash, Glob, TodoWrite, Task, mcp__codeb-deploy__workflow_init, mcp__codeb-deploy__workflow_scan, mcp__codeb-deploy__deploy_project, mcp__codeb-deploy__slot_status, mcp__codeb-deploy__slot_promote, mcp__codeb-deploy__health_check, mcp__codeb-deploy__domain_setup, mcp__codeb-deploy__domain_list]
description: "팀원용 통합 명령어 - 스캔 → 설정 → 배포 자동화"
---

# /we:quick - 팀원용 통합 명령어 (v7.0)

## 🎯 목적
팀원이 **한 번의 명령어로** 프로젝트 설정부터 배포까지 완료.
스캔으로 신규/기존 자동 감지 → 필요한 작업만 수행.

## 사용법
```
/we:quick [프로젝트명] [--env staging|production]
```

## 실행 흐름

```
┌─────────────────────────────────────────────────────────────────┐
│                    /we:quick 실행 흐름                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1️⃣ 스캔 (항상 먼저)                                            │
│     ├─ SSOT에 프로젝트 있나?                                    │
│     ├─ App서버에 ENV 있나?                                      │
│     ├─ Storage서버에 DB/Redis 있나?                             │
│     └─ 도메인 설정 있나?                                        │
│                                                                 │
│  2️⃣ 분기                                                        │
│     ├─ 신규 → 서버별 리소스 생성                                │
│     └─ 기존 → 현재 상태 출력                                    │
│                                                                 │
│  3️⃣ 배포 (선택)                                                 │
│     └─ 사용자 확인 후 배포                                      │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

## 1단계: 스캔

```
로컬:
├─ package.json → 프로젝트명, 타입 감지
└─ .github/workflows/ → 워크플로우 존재 여부

서버 (MCP 통해):
├─ App (158.247.203.55)
│  ├─ /opt/codeb/registry/ssot.json → 프로젝트 등록 여부
│  └─ /opt/codeb/env/{project}/ → ENV 파일 존재 여부
│
└─ Storage (64.176.226.119)
   ├─ PostgreSQL → {project}_db 존재 여부
   └─ Redis → DB 번호 할당 여부
```

## 2단계: 신규 프로젝트 → 서버별 생성

```
┌─────────────────────────────────────────────────────────────────┐
│                    신규 프로젝트 자동 생성                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  App Server (158.247.203.55)                                    │
│  ├─ SSOT 등록: 프로젝트명, 포트 할당                            │
│  ├─ ENV 생성: /opt/codeb/env/{project}/staging/.env            │
│  ├─ ENV 생성: /opt/codeb/env/{project}/production/.env         │
│  ├─ ENV 백업: /opt/codeb/env-backup/{project}/                 │
│  └─ Caddy 도메인: {project}.codeb.kr                           │
│                                                                 │
│  Storage Server (64.176.226.119)                                │
│  ├─ PostgreSQL: CREATE DATABASE {project}_db                   │
│  ├─ PostgreSQL: CREATE USER {project}_user WITH PASSWORD       │
│  └─ Redis: DB 번호 할당 (SSOT에서 관리)                         │
│                                                                 │
│  GitHub Repository (자동 감지)                                   │
│  ├─ GHCR_PAT: gh auth token으로 자동 설정                       │
│  └─ CODEB_API_KEY: 팀원 API Key 자동 설정                       │
│                                                                 │
│  결과 → ENV 파일에 자동 반영:                                    │
│  DATABASE_URL=postgres://{user}:{pass}@db.codeb.kr:5432/{db}   │
│  REDIS_URL=redis://db.codeb.kr:6379/{db_num}                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

## 2-1단계: GitHub Secrets 자동 설정

**신규 프로젝트 초기화 후 반드시 실행:**

```bash
# 1. GitHub repo 감지
REPO=$(git remote get-url origin 2>/dev/null | sed 's/.*github.com[:/]\(.*\)\.git/\1/' | sed 's/.*github.com[:/]\(.*\)/\1/')

# 2. GHCR_PAT 설정 (현재 gh 토큰 사용)
gh secret set GHCR_PAT --repo "$REPO" --body "$(gh auth token)"

# 3. CODEB_API_KEY 설정 (~/.codeb/config.json에서 읽기)
API_KEY=$(cat ~/.codeb/config.json 2>/dev/null | jq -r '.apiKey // empty')
if [ -n "$API_KEY" ]; then
  gh secret set CODEB_API_KEY --repo "$REPO" --body "$API_KEY"
fi

# 4. 확인
gh secret list --repo "$REPO"
```

**실행 조건:**
- `git remote` 가 github.com을 가리킬 때만 실행
- `gh auth status`로 로그인 확인 후 실행
- 이미 설정된 시크릿은 덮어쓰기 (최신 토큰으로 갱신)

## 2단계: 기존 프로젝트 → 상태 출력

```
┌─────────────────────────────────────────────────────────┐
│  📊 myapp 현재 상태                                      │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  SSOT: ✅ 등록됨                                        │
│  포트: staging 4500/4501, production 4100/4101         │
│                                                         │
│  Staging                                                │
│  ├─ Blue:  ✅ active  (v1.2.3)                         │
│  └─ Green: ⬚ empty                                     │
│                                                         │
│  Production                                             │
│  ├─ Blue:  ⏸ grace   (v1.2.2)                          │
│  └─ Green: ✅ active  (v1.2.3)                         │
│                                                         │
│  ENV: ✅ staging, ✅ production                         │
│  DB:  ✅ myapp_db                                       │
│  도메인: myapp.codeb.kr                                 │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

## 3단계: 배포

```
배포 방법:
1. git push (권장) → 워크플로우 자동 실행
2. /we:quick --deploy → MCP 통해 직접 배포
```

## Git 워크플로우 룰

```
main/master  → production 자동 배포
develop      → staging 자동 배포
feature/*    → preview 환경 (PR)
```

워크플로우는 SSOT 파일에서 포트를 읽음:
```yaml
- name: Get port from SSOT
  run: |
    PORT=$(jq -r '.projects["'$PROJECT'"].ports.production.blue' /opt/codeb/registry/ssot.json)
```

## SSOT 구조

```json
{
  "version": "7.0",
  "projects": {
    "myapp": {
      "type": "nextjs",
      "ports": {
        "staging": { "blue": 4500, "green": 4501 },
        "production": { "blue": 4100, "green": 4101 }
      },
      "database": { "name": "myapp_db", "user": "myapp_user" },
      "redis": { "db": 1 },
      "domain": "myapp.codeb.kr"
    }
  },
  "ports": { "used": [4100, 4101, 4500, 4501] }
}
```

## 포트 범위

| 환경 | 범위 | 용도 |
|------|------|------|
| production | 4100-4499 | Blue-Green |
| staging | 4500-4999 | Blue-Green |
| preview | 5000-5499 | PR Preview |

## 팀원용 .env (최소 필수)

```bash
CODEB_API_URL=https://api.codeb.kr
CODEB_API_KEY=codeb_codeb_member_xxxxx
```

## Admin 전용 명령어

- `/we:init` - 서버 인프라 직접 관리
- `/we:workflow` - CI/CD 상세 관리
- `/we:domain` - 도메인 설정
- `/we:registry` - SSOT 직접 수정
- `/we:ssh` - SSH 키 관리
