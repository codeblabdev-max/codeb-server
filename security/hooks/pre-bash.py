...
def get_allowed_servers():
    ...
    if cache and "servers" in cache:
        return cache["servers"].get("ips", CONFIG["default_allowed_ips"])

    return CONFIG["default_allowed_ips"]
...
def check_ssh_target(command):
    """SSH 대상 검증"""
    allowed_ips = get_allowed_servers()
...
def check_bypass_attempts(command):
    ...
    bypass_patterns = [
        ...
        # systemd 서비스 조작
        (r"systemctl\s+.*codeb-protection", "보호 데몬 서비스 조작 시도 감지"),
    ]
